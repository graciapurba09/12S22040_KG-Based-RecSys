...
class KGRS:
    def __init__(self, train_pos: np.array, train_neg: np.array, kg_lines: List[str], n_user: int, n_item=int):
        ...
        config = {
        "batch_size": 256,         # Naikkan sedikit agar stabilitas gradien meningkat (lebih efisien di GPU)
        "eval_batch_size": 2048,   # Evaluasi lebih cepat di batch besar
        "neg_rate": 3,             # Tambah sedikit untuk memperkaya contoh negatif → mendorong pemisahan lebih kuat
        "emb_dim": 384,            # Lebih besar untuk menangkap representasi kompleks tanpa terlalu besar
        "l1": True,                # Tetap L1, bagus untuk sparsity
        "margin": 8,               # Turunkan sedikit agar training tidak terlalu keras (lebih halus)
        "learning_rate": 3e-4,     # Sedikit lebih rendah untuk training yang lebih stabil
        "weight_decay": 5e-4,      # Sedikit dikurangi supaya model tidak underfit
        "epoch_num": 150,          # Tambah epoch agar model sempat konvergen di LR yang lebih rendah       
        }
